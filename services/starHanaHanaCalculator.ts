// services/starHanaHanaCalculator.ts
import type { StarHanaHanaInput, StarHanaHanaSettingProbabilities, StarHanaHanaFullResult, StarHanaHanaProbabilitiesBreakdown, ObservedRates } from '../types';
import { KingHanaHanaSideLampColor } from '../types';
import { 
  StarHanaHanaSetting,
  STAR_HANA_HANA_SETTINGS_NAMES,
  STAR_HANA_HANA_IDEAL_RATES,
  GAMES_PER_BIG_BONUS_STAR_HANA,
  SHH_REG_DURING_SIDE_LAMP_PROBS,
  SHH_BIG_AFTER_FEATHER_LAMP_PROBS,
  SHH_REG_AFTER_FEATHER_LAMP_PROBS
} from '../constants/starHanaHanaConstants';

const MIN_LIKELIHOOD = 1e-10;

const normalizeProbabilities = (probs: StarHanaHanaSettingProbabilities): StarHanaHanaSettingProbabilities => {
  const normalized: StarHanaHanaSettingProbabilities = {};
...
export const calculateStarHanaHanaProbabilities = (
  inputs: StarHanaHanaInput
): StarHanaHanaFullResult => {
  const overallLikelihoods: StarHanaHanaSettingProbabilities = {};
  const breakdownLikelihoods: StarHanaHanaProbabilitiesBreakdown = {};
  ...
  STAR_HANA_HANA_SETTINGS_NAMES.forEach(setting => {
  ...
  const totalBigGamesForSubCounters = playedBigCount * GAMES_PER_BIG_BONUS_STAR_HANA;
  ...
  // --- Likelihood Calculation Loop (similar to KingHanaHanaS, using SHH constants) ---
   elementNames.forEach(elementName => {
    const tempLogLikelihoods: StarHanaHanaSettingProbabilities = {};
    const isElementActive = activeElementKeys.has(elementName);

    STAR_HANA_HANA_SETTINGS_NAMES.forEach(setting => {
      const idealRates = STAR_HANA_HANA_IDEAL_RATES[setting as StarHanaHanaSetting];
      let logL = 0; 
      if (isElementActive) {
        switch (elementName) {
          case "BIG確率":
          ...
          case "BIG中スイカ 確率":
            {
              const n_big_sub = playedBigCount * GAMES_PER_BIG_BONUS_STAR_HANA;
          ...
          case "REG中サイドランプ": {
            ...
            Object.values(KingHanaHanaSideLampColor).filter(c => c !== KingHanaHanaSideLampColor.NONE).forEach(color => {
              const key = `regDuringSideLamp${color.replace('青🔵','Blue').replace('黄🟡','Yellow').replace('緑🟢','Green').replace('赤🔴','Red').replace('虹🌈','Rainbow')}Count` as keyof StarHanaHanaInput;
            ...
            if (sumOfObservedLamps > 0) {
              ...
              else logL = getLogLikelihoodForMultinomial(lampCounts, sumOfObservedLamps, SHH_REG_DURING_SIDE_LAMP_PROBS[setting as StarHanaHanaSetting]);
            } else if (playedRegCount > 0) {
              logL = getLogLikelihoodForMultinomial(lampCounts, playedRegCount, SHH_REG_DURING_SIDE_LAMP_PROBS[setting as StarHanaHanaSetting]);
            } else logL = 0;
            break;
          }
          case "BIG後フェザーランプ": {
            const lampCounts: Record<string, number> = {};
            Object.values(KingHanaHanaSideLampColor).filter(c => c !== KingHanaHanaSideLampColor.NONE).forEach(color => {
              const key = `bigAfterSideLamp${color.replace('青🔵','Blue').replace('黄🟡','Yellow').replace('緑🟢','Green').replace('赤🔴','Red').replace('虹🌈','Rainbow')}Count` as keyof StarHanaHanaInput;
              lampCounts[color] = inputs[key] as number || 0;
            });
            ...
            else if (logL !== -Infinity) logL = getLogLikelihoodForMultinomial(lampCounts, currentOpportunities, SHH_BIG_AFTER_FEATHER_LAMP_PROBS[setting as StarHanaHanaSetting]);
            break;
          }
          case "REG後フェザーランプ": {
             const lampCounts: Record<string, number> = {};
             Object.values(KingHanaHanaSideLampColor).filter(c => c !== KingHanaHanaSideLampColor.NONE).forEach(color => {
              const key = `regAfterSideLamp${color.replace('青🔵','Blue').replace('黄🟡','Yellow').replace('緑🟢','Green').replace('赤🔴','Red').replace('虹🌈','Rainbow')}Count` as keyof StarHanaHanaInput;
              lampCounts[color] = inputs[key] as number || 0;
            });
            const sumOfLamps = Object.values(lampCounts).reduce((s, c) => s + c, 0);
            if (sumOfLamps > playedRegCount && playedRegCount > 0) logL = -Infinity;
            else if (sumOfLamps > 0 && playedRegCount === 0) logL = -Infinity;
            else if (logL !== -Infinity) logL = getLogLikelihoodForMultinomial(lampCounts, playedRegCount, SHH_REG_AFTER_FEATHER_LAMP_PROBS[setting as StarHanaHanaSetting]);
            break;
          }
          case "開始時データ":
            if (inputs.startTotalGames > 0) {
  ...
  const finalResult: StarHanaHanaFullResult = {
  ...
  if (inputs.currentNetMedals !== null && playedGames > 0) {
    ...
    const idealRatesS1 = STAR_HANA_HANA_IDEAL_RATES[StarHanaHanaSetting.SETTING_1];
